...
// substituted ADO.NET provider (a fakeDbConnection here, but the same is true for any non-DuckDB
// provider substitute), that cast throws InvalidCastException for every single mapped parameter,
// regardless of its .NET type. These tests prove the fix: a fakeDbConnection can bind a WHERE-clause
// parameter of each affected .NET type without that cast ever firing. No real DuckDB engine is
...
public class FakeDbTypeMappingTests
{
    ...
    {
        var connection = (fakeDbConnection)new fakeDbFactory(SupportedDatabase.DuckDB).CreateConnection()!;
    ...
    public async Task SaveChangesAsync_inserts_a_row_carrying_bound_parameters_of_every_affected_type()
    {
        ...
        connection.EnqueueReaderResult(Array.Empty<Dictionary<string, object?>>(), recordsAffected: 1);
    ...
    private static void AssertBoundParameterValue(fakeDbConnection connection, object expectedValue)
    {
        var command = connection.ExecutedReaderCommands.Single();
        Assert.Contains(command.Parameters, p => Equals(p.Value, expectedValue));
    }

    private static WidgetContext CreateContext(fakeDbConnection connection)
    {
