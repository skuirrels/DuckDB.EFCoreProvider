...
namespace DuckDB.EFCoreProvider;

// Vendored from the Entity Framework Core source (dotnet/efcore) — its SharedTypeExtensions is an internal
// helper that is not exposed as a public API, so it is copied here rather than referenced. Keep changes
// minimal so it can be diffed against the upstream original.
